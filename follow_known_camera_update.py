...
class FollowKnownCamera:
    ...
    def __init__(self):
        ...
        # FIX: When target is lost, robot stops immediately and does NOT move forward.
        # lost_timeout is only used to reset state if Viheet doesn't reappear.
        self.lost_timeout = rospy.get_param("~lost_timeout", 2.0)
        ...
        # State machine:
        # SEARCHING: scan all faces and recognise Viheet
        # FOLLOWING: follow only Viheet, re-verify every frame
        self.state = "SEARCHING"
        self.has_greeted = False

        # FIX: Track whether Viheet is currently visible in this frame
        ...
        rospy.loginfo("Follow node started. Target: {}".format(self.target_person))
        rospy.loginfo("Image topic: {}".format(self.image_topic))
        rospy.loginfo("Velocity topic: {}".format(self.cmd_vel_topic))
        rospy.loginfo("Known names loaded: {}".format(self.known_names))
    ...
    def load_face_detector(self):
        ...
        rospy.loginfo("Loaded Haar face detector from: {}".format(cascade_path))
    ...
    def control_timer(self, event):
        ...
        # FIX: If following but Viheet is NOT visible in the current frame,
        # stop completely. Do not move forward at all.
        if self.state == "FOLLOWING" and not self.target_visible:
            time_since_seen = (rospy.Time.now() - self.last_seen_time).to_sec()
            rospy.logwarn_throttle(
                1.0,
                "Target not visible. Stopped. ({:.1f}s since last seen)".format(time_since_seen)
            )
            # Hard stop — publish zero velocity every tick while waiting
            self.target_twist = Twist()
            self.current_twist = Twist()
            self.cmd_pub.publish(Twist())

            # After lost_timeout seconds with no sighting, go back to searching
            if time_since_seen > self.lost_timeout:
                rospy.logwarn("Target lost for {:.1f}s. Returning to SEARCHING.".format(
                    self.lost_timeout))
                self.state = "SEARCHING"
                self.confirm_count = 0
                self.has_greeted = False
        ...
        if self.front_min_distance < self.safe_stop_distance:
            rospy.logwarn_throttle(
                1.0,
                "Obstacle too close: {:.2f} m. HARD STOP.".format(self.front_min_distance)
            )
            self.hard_stop()
            return
    ...
    def image_callback(self, msg):
        ...
        # ---------------------------------------------------------------
        # SEARCHING STATE
        # FIX: Check ALL detected faces, not just the largest one,
        # so Viheet isn't missed if someone else is closer to the camera.
        # ---------------------------------------------------------------
        if self.state == "SEARCHING":
            ...
            if len(faces) == 0:
                rospy.loginfo_throttle(1.0, "SEARCHING: no faces detected.")
                return
            ...
            for (x, y, w, h) in faces:
                ...
                rospy.loginfo_throttle(
                    1.0,
                    "SEARCHING: face candidate={} score={:.1f}".format(name, score)
                )

                if name == self.target_person and score <= self.match_threshold:
                    found_target = True
                    target_box = (x, y, w, h)
                    break  # Found Viheet, no need to check other faces
        ...
        # ---------------------------------------------------------------
        # FOLLOWING STATE
        # FIX: Re-verify identity every single frame.
        # If Viheet is not found among all faces, stop immediately.
        # Never follow an unverified face.
        # ---------------------------------------------------------------
        if self.state == "FOLLOWING":

            if len(faces) == 0:
                # No faces at all — stop and wait
                self.target_visible = False
                rospy.logwarn_throttle(1.0, "FOLLOWING: no faces in frame. Stopped.")
                return

            # Search all detected faces for Viheet
            viheet_box = None
            best_score_found = 999.0
            ...
            for (x, y, w, h) in faces:
                ...
                if name == self.target_person and score <= self.match_threshold:
                    # If multiple matches (unlikely), take the best scoring one
                    if score < best_score_found:
                        best_score_found = score
                        viheet_box = (x, y, w, h)

            if viheet_box is None:
                # Someone else is in frame but NOT Viheet — stop, do not follow
                self.target_visible = False
                rospy.logwarn_throttle(
                    1.0,
                    "FOLLOWING: faces detected but none matched {}. Stopped.".format(
                        self.target_person)
                )
                return

            # Viheet confirmed in this frame
            self.target_visible = True
            self.last_seen_time = rospy.Time.now()
            ...
            if abs(error_x) < self.dead_zone_px:
                ...
                new_target.angular.z = self.limit(
                    raw_turn, -self.max_turn_speed, self.max_turn_speed
                )

            # Forward motion — only if Viheet is visible and centred
            if self.front_min_distance < self.safe_stop_distance:
            ...
            rospy.loginfo_throttle(
                1.0,
                "FOLLOWING {}: score={:.1f} face_h={} error_x={:.1f} "
                "front={:.2f}m vx={:.2f} wz={:.2f}".format(
                    self.target_person,
                    best_score_found,
                    h,
                    error_x,
